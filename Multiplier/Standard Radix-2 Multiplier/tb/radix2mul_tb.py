...
async def perform_multiplication(dut, multiplicand: int, multiplier: int):
    ...
    # Wait until it reaches STOP state or wait at least N + 5 (37) clock cycles before moving on to the next step
    # since we need 34 cycles to run (2 cycles for IDLE/STOP, 32 cycles for CALC). I choose 37 just to be safe.
    for _ in range(N + 5):
        await RisingEdge(dut.clk)
